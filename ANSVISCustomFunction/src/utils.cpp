...
std::wstring lite::utils::to_wstring(const std::string& str)
{
    ...
    size_t converted = 0;
    mbstowcs_s(&converted, p, len, str.c_str(), len - 1);
    std::wstring wstr(p);
    delete[] p;
    return wstr;
}
...
void lite::utils::draw_boxes_with_landmarks_inplace(cv::Mat& mat_inplace, const std::vector<types::CustomObjectType>& boxes_kps, bool text)
{
    if (boxes_kps.empty()) return;
    for (const auto& box_kps : boxes_kps)
    {
        if (box_kps.flag)
        {
            ...
            if (box_kps.flag)
            {
                cv::rectangle(mat_inplace, box_kps.box, cv::Scalar(255, 255, 0), 2);
                if (!box_kps.className.empty() && text)
                {
                    std::string label_text(box_kps.className);
                    if (label_text == "Unknown")
                    {
                        label_text = "";
                    }
